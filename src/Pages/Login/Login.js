...
const GET_SUPERADMIN_BY_EMAIL = gql`
  query GetSuperAdminByEmail($email: String!, $password: String!) {
    getSuperAdminByEmail(email: $email, password: $password) {
      _id
      name
      email
      isAdmin
...
const Login = () => {
  ...
  const [getSuperAdminByEmail, { data, loading, error }] = useLazyQuery(
    GET_SUPERADMIN_BY_EMAIL
  );
  const handleSubmit = (event) => {
    event.preventDefault();
    getSuperAdminByEmail({ variables: { email, password } });
  ...
  useEffect(() => {
    if (data) {
      ...
      navigate("/dashboard", {
        state: { superAdmin: data.getSuperAdminByEmail },
      });
